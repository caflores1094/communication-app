/**
 * Splits up a template string into an array where each element is either a string original to the template string or the value of it's corresponding variable mappped by varMap
 * @param   {String} templateString the template string to be parsed
 * @param   {Object} varMap the map holding the variables to be subsituted in
 * @returns {Array}
 * Usage:
...
export const toLocaleNumbering = (number) => {
    ...
    return convertedNumber.toLocaleString(process.env.lang[process.env.lang]);
};

export const correctDateFormatting = (date) => {
    return date.toLocaleString(process.env.lang[process.env.lang]);
};
